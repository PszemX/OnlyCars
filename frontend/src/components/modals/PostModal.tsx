...
export const PostModal = ({
  ...
  useEffect(() => {
    const fetchComments = async () => {
      const data = await apiFetch(`http://localhost:5001/api/posts/${post.id}/comments`, {
        method: "GET",
      });
      setComments(data);
      ...
      const userMap = data.reduce((acc: any, comment: Comment, index: number) => {
  ...
  const addComment = async () => {
    ...
    try {
      ...
      const currentUser = await apiFetch(`http://localhost:5001/api/users/current`, {
        method: "GET",
      });
